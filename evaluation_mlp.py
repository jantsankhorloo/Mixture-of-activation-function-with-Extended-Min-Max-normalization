...
activations = ['relu', 'linear', 'swish','sigmoid', 'tanh','sine', 'cos']
#activations = [ 'swish','sigmoid']
#activations = ['tanh','sine', 'cos']
    ...
    for activation in activations:
        ...
        for train_index, test_index in tscv.split(train):
            ...
            if activation in ['relu', 'linear', 'swish','sigmoid']:
                #training, test = normalization.sklearn_normalization(features=[0, 1])
                training, test = normalization.Normalization(features=[0, 1])
            else:
                #training, test = normalization.sklearn_normalization(features=[-1, 1])
            ...
            modelling = simple_MLP_model(trainingX=trainX,  valX=valX, trainingY=trainY, valY=valY,
                                         activation=activation, output_activation=activation,
                                         mlp_layers =[32], time_steps=5, batch_size=64,
            ...
            final_model = modelling.mlp_model()
